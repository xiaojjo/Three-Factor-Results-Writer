...
- [工作流](#工作流)
- [决策树](#决策树)
- [多水平算法（K>2 时触发）](#多水平算法k2-时触发)
- [风格规范（要点）](#风格规范要点)
- [md2docx.py 用法](#md2docxpy-用法)
- [示例](#示例)
- [许可证](#许可证)
...
```mermaid
flowchart TD
    START[单指标] --> Q0{factor_count = 1?}
    Q0 -->|是| R3[R3 仅主效应<br>直接极值对比]
    Q0 -->|否| Q1{最高阶交互 P < 0.05?}
    Q1 -->|是| R1[R1 最高阶交互显著<br>拆解 k 个方向]
    Q1 -->|否| Q2{≥1 个次高阶交互 P < 0.05?}
    Q2 -->|是| R2[R2 次高阶交互显著<br>每个交互双向拆解]
    Q2 -->|否| Q3{≥1 个主效应 P < 0.05?}
    Q3 -->|是| R3
    Q3 -->|否| R4[R4 全不显著<br>仅描述性报告]
```

> 下方为等价的纯文本决策树（无需 Mermaid 渲染即可阅读）：

```
单指标
└─ factor_count == 1 ?
   ├─ 是 → R3 仅主效应：直接极值对比（Level_K vs Level_1）
   └─ 否 → 最高阶交互 P < 0.05 ?
      ├─ 是 → R1 最高阶交互显著：按 k 个方向拆解
      │         （每方向：统计分析 → 趋势分析 → 效应说明）
      └─ 否 → ≥1 个次高阶交互 P < 0.05 ?
         ├─ 是 → R2 次高阶交互显著：每个交互双向拆解
         └─ 否 → ≥1 个主效应 P < 0.05 ?
            ├─ 是 → R3 仅主效应：极值对比
            └─ 否 → R4 全不显著：仅描述性报告
```
...
零依赖（ Python 3 标准库），自动生成同名 `.docx`，套用期刊排版（宋体 + TNR 12pt 正文、黑体标题、A4 页面、1.5 倍行距）。
